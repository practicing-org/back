...
export default async(req:Request, res:Response, next:NextFunction)=>{
    ...
    try{
        const query = 'select user_Id, name, genderId from friend inner join user using(user_Id) where user_Id not in'+userIds+'friend =:user_Id and user_Id = ANY(select friend from friend where user_Id =:user_Id) desc limit 10'
